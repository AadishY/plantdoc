    ...
    <section className="py-8 md:py-12 container mx-auto px-4 relative z-10 max-w-6xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {METRICS.map((item, idx) => (
          <SpotlightCard key={idx} className="p-7 text-left flex flex-col justify-between min-h-[240px]">
            <div>
              <div className="w-12 h-12 rounded-2xl bg-[#2DD4BF]/15 border border-[#2DD4BF]/30 flex items-center justify-center mb-5 shadow-[0_0_15px_rgba(45,212,191,0.25)]">
                {item.icon}
              </div>
              <div className="text-3xl sm:text-4xl font-mono font-extrabold text-white tracking-tight mb-1">
                {item.value}
              </div>
              <div className="text-sm font-bold text-[#5EEAD4] mb-2">
                {item.label}
              </div>
            </div>
